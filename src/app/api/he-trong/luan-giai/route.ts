...
import {
  HE_TRONG_PROMPT_VERSION,
  buildHeTrongContext,
  buildHeTrongSystemPrompt,
  buildHeTrongUserBlock,
  parseHeTrongPayload,
} from '@/lib/fengshui/he-trong-prompt';
import {
  aiDeviceSetCookie,
  consumeAiCredit,
  getAiIdentity,
...
export async function POST(req: Request) {
  ...
  const payload = parseHeTrongPayload(raw);
  if (!payload) {
    return Response.json(
      { error: 'Dữ liệu xem chưa hợp lệ. Vui lòng kiểm tra lại ngày và năm sinh.' },
      { status: 400 },
    );
  }
  ...
  const ctx = buildHeTrongContext(payload);
  const advisor = isSimSite ? sitePersona!.displayName : 'trụ trì';
  const messages = [
    {
      role: 'system',
      content: buildHeTrongSystemPrompt(
        templeName,
        isSimSite
          ? {
    ...
    { role: 'user', content: buildHeTrongUserBlock(ctx, advisor) },
  ];
      ...
      try {
        ...
        if (full.trim().length >= MIN_CACHEABLE_CHARS) {
          await saveCachedAiAnswer({
            ...
            content: full.trim(),
            model: MODEL,
            promptVersion: HE_TRONG_PROMPT_VERSION,
          });
        }
        try {
          controller.close();
