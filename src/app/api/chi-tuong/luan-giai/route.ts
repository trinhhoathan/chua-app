...
import {
  CHI_TUONG_PROMPT_VERSION,
  buildChiTuongContext,
  buildChiTuongSystemPrompt,
  buildChiTuongUserBlock,
  parseChiTuongPayload,
} from '@/lib/fengshui/chi-tuong-prompt';
import {
  aiDeviceSetCookie,
  consumeAiCredit,
  getAiIdentity,
...
export async function POST(req: Request) {
  ...
  const payload = parseChiTuongPayload(raw);
  if (!payload) {
    return Response.json(
  ...
  const ctx = buildChiTuongContext(payload);
  const advisor = isSimSite ? sitePersona!.displayName : 'trụ trì';
  const messages = [
    {
      role: 'system',
      content: buildChiTuongSystemPrompt(
        templeName,
        isSimSite
          ? {
    ...
    { role: 'user', content: buildChiTuongUserBlock(ctx, advisor) },
  ];
      ...
      try {
        ...
        if (full.trim().length >= MIN_CACHEABLE_CHARS) {
          await saveCachedAiAnswer({
            ...
            content: full.trim(),
            model: MODEL,
            promptVersion: CHI_TUONG_PROMPT_VERSION,
          });
        }
        try {
          controller.close();
